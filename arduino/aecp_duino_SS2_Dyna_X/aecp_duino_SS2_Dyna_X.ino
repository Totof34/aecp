...
char ver[] = "version du 02_07_2020";//Choix entre 3 types de Dwell.
...
const int Avancestatique = 22; // Correspond à 4 dents d'avance statique sur volant moteur
int Decalagedemarrage = 8; // viens en soustraction de l'avance statique pour faciliter le démarrage, ici 22 - 8 = 14°
...
const int Nplancher = 700; // vitesse en t/mn jusqu'a laquelle l'avance  = 0°
...
{ AngleCibles = 720 / Ncyl; //Ex pour 4 cylindres 180°, et 120° pour 6 cylindres
  ...
  RDzero = float(AngleCapteur - Avancestatique + Decalagedemarrage) / float(AngleCibles);
  ...
  Serial.print("Tc = "); for (int i = 1 ; i < 12; i++)Serial.println(Tc[i]);
  Serial.print("Tlim = "); Serial.println(Tlim);
  Serial.print("C1 = "); for (int i = 1 ; i < 12; i++)Serial.println(C1[i]);
  Serial.print("C2 = "); for (int i = 1 ; i < 12; i++)Serial.println(C2[i]);
...
////////////////////////////////////////////////////////////////////////////
{
  ...
  //digitalWrite(Led,LOW); // Décommenter cette ligne pour caller le capteur , voir plus loin la 2ème ligne 416
  Dep = analogRead(A0);
