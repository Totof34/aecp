...
//http://www.loutrel.org/aeduino.php
//Mr Dedessus Les Moutier Christophe pour la version Panhard
char ver[] = "version du 21_07_2025";//Choix entre 3 types de Dwell.
...
//**********************************************************************************
const int Dwell = 3 ; //1 pour alimenter la bobine en permanence sauf 1ms/cycle.Elle doit pouvoir chauffer sans crainte
...
//*******//*********Courbe   b
int Nb[] = {0, 1640, 2000, 2480, 3800, 4000, 4120,4240, 4360, 7000, 0}; //Courbe b 
int Angb[] = {0, 0, 2, 4, 10, 11, 12, 13, 14, 14, 0};
//*******//*********Courbe   c
int Nc[] = {0,840, 2100, 3000, 3400, 3760, 3880, 3950, 4000, 7000, 0}; //Courbe c
int Angc[] = {0, 0, 6, 10, 12, 14, 15, 16, 18, 18, 0};
//*******//*********Courbe   d
int Nd[] = {0, 1640, 2520, 2840, 3760, 3880, 3990, 4000, 7000, 0}; //Courbe d
int Angd[] = {0, 0, 5, 7, 14, 15, 16, 18, 18, 0};
//*******//*********Courbe   e
int Ne[] = {0, 840, 2000, 2580, 3520, 3800, 4000, 4120, 4240, 4360, 7000, 0}; //Courbe e
int Ange[] = {0, 0, 4, 6, 9, 10, 11, 12, 13, 14, 14, 0};
//**********************************************************************************
//********************FLASH********************
...
const int Pot1 = A4;   //Entrée analogique sur A4 pour potard de changement de courbes. R PullUp
...
int unsigned long Vitesse = 0; //Vitesse de rotation moteur
...
char  carLu;//Sera utilisé pour changer de courbe
...
void Change_de_Courbe()///////////////////

{ // Avec a,b ou c on change de courbe, sinon delta de +1 deg à +6 deg  ou negatif 11 à 16
  while (BT.available() > 0) //Nb de car dans le buffer venant du sphone
  { carLu = BT.read(); //oui il y a un car au moins
    //Serial.println(carLu);
    
  }
  if (carLu != "") //Une valeur a été entrée au sphone
  {
    
  if ((carLu=='a')||(carLu=='b')||(carLu=='c')||(carLu=='d')||(carLu=='e')){
  Select_Courbe_sphone();  //Ajuster éventuellement les pointeurs pN et pA pour la courbe b ou c 
  Prep_courbe();
  }
  if ((carLu=='f')||(carLu=='g')||(carLu=='h')||(carLu=='i')||(carLu=='j')){
  Select_Courbe_dep_sphone();
  }
...
{ valPot2 = analogRead(Pot2);
  ...
  if (valPot2 < 99) {                  // Shunt 0 ohm donne 15 
    ...
    yhigh = 300; // soit 30° , 28° pour ma 24B
    ylow = 0;    // soit 0°
    Serial.println(" Courbe a");
  }
  ...
  if (valPot2 > 715 && valPot2 < 755) {  // Résistance de 100K donne 735    
    xhigh = 305; // soit 210 mmHg pour essai 225 mmhg
    xlow = 538;  // soit 60 mmHg pour essai 95 mmhg
    yhigh = 280; // soit 28° 28° pour essai 24B
    ylow = 0;    // soit 0°
    Serial.println(" Courbe e");
  }
  if (valPot2 > 995) {                  // Pas de shunt donne 1015 
    xhigh = 330; // soit 210 mmHg = 330
    xlow = 565;  // soit 80 mmHg = 565
    yhigh = 300; // soit 30°
    ylow = 0;    // soit 0°
    Serial.println(" Courbe a");
  }
...
void  Select_Courbe_sphone()////////////while (1); delay(1000);/////////////////////////////
//Par défaut, la courbe a est déja selectionnée
{ 
  if ( carLu == 'a') {                  // Shunt 0 ohm donne 15 
    pN = &Na[0];  // pointer à la courbe a
    pA = &Anga[0];
    Serial.println("Courbe a");
  }
  if ( carLu == 'b') { // Résistance de 4K7 donne 130 
    pN = &Nb[0];  // pointer à la courbe b
    pA = &Angb[0];
    Serial.println("Courbe b");
  }
  if ( carLu == 'c'){  // Résistance de 18K donne 340    
    pN = &Nc[0];  // pointer à la courbe c
    pA = &Angc[0];
    Serial.println("Courbe c");
  }
  if ( carLu == 'd') {  // Résistance de 47K donne 565    
    pN = &Nd[0];  // pointer à la courbe d
    pA = &Angd[0];
    Serial.println("Courbe d");
  }
  if ( carLu == 'e') {  // Résistance de 100K donne 735    
    pN = &Ne[0];  // pointer à la courbe e
    pA = &Ange[0];
    Serial.println("Courbe e");
  }
...
void  Select_Courbe_dep_sphone()////////////while (1); delay(1000);/////////////////////////////
//Par défaut, la courbe a est déja selectionnée
{ 
    if ( carLu == 'f' ){                  // Shunt 0 ohm donne 15 
    xhigh = 330; // soit 210 mmHg = 330
    xlow = 565;  // soit 80 mmHg = 565
    yhigh = 300; // soit 30°
    ylow = 0;    // soit 0°
    Serial.println("Courbe f");
  }
  if ( carLu == 'g' ) { // Résistance de 4K7 donne 130 
    xhigh = 330; // soit 210 mmHg
    xlow = 538;  // soit 95 mmHg
    yhigh = 280; // soit 28°
    ylow = 0;    // soit 0°
    Serial.println("Courbe g");
  }
  if ( carLu == 'h' ) {  // Résistance de 18K donne 340    
    xhigh = 350; // soit 200 mmHg
    xlow = 601;  // soit 60 mmHg
    yhigh = 320; // soit 32°
    ylow = 0;    // soit 0°
    Serial.println("Courbe h");
  }
  if ( carLu == 'i' ) {  // Résistance de 47K donne 565    
    xhigh = 350; // soit 200 mmHg
    xlow = 538;  // soit 95 mmHg
    yhigh = 320; // soit 32°
    ylow = 0;    // soit 0°
  ...
  if ( carLu == 'j' ) {  // Résistance de 100K donne 735    
    xhigh = 330; // soit 210 mmHg
    xlow = 601;  // soit 60 mmHg
    yhigh = 280; // soit 28°
    ylow = 0;    // soit 0°
    Serial.println("Courbe j");
  }
}
////////////////////////////////////////////////////////////////////////
...
////////////////////////////////////////////////////////////////////////////
{
  ...
  //digitalWrite(Led,HIGH); // Décommenter cette ligne pour caller le capteur , voir plus loin la 2ème ligne 531
  Dep = analogRead(A0);
