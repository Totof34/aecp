...
public void drawmycentrifugalcurve(){
  for (int q = 1; q<175; q++){
    ...
    float yr = myspeed [q];    
    if (stateCent == false ){
    line(xcentOrigin+lastxr,ycentOrigin-20-lastyr,xcentOrigin+xr,ycentOrigin-20-yr); 
    }
    else {
    fill(0);
...
public void drawcentrifugalcurve(){
  ...
  if (stateCent == false){
  text(0,xcentOrigin-15,635);
  text(1,xcentOrigin-15,615);
  text(3,xcentOrigin-15,575);
  text(5,xcentOrigin-15,535);
  ...
  // Point d'origine pour les courbe (480,680)
   ...
   if (stateCent == false ){
   line((xs),(ydInt-30),(lastxs),(lastydInt-30));
   if (ydInt<650){
   line((xs),(ydInt-10),(lastxs),(lastydInt-10));
   }
   }
   else
   {
