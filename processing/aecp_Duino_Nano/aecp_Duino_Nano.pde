...
public void drawmycentrifugalcurve(){
  for (int q = 1; q<175; q++){
    ...
    //println(yr);
    fill(0);
    line(390+lastxr,650-lastyr,390+xr,650-yr);
    //println(q);
  }
}
...
public void drawcentrifugalcurve(){
  ...
  // Point d'origine pour les courbe (480,680)
   ...
   int lastxs = 390 + (lastspeedData/20);
   float lastyd = 650 - (lastdegreeData*20);
   int lastydInt = int(lastyd);
   
   fill(0,255,0);
   //line(xs,ydInt,xs+1,ydInt-1); // courbe cible d'origine
   
   line((xs*0.98),(ydInt*0.98),(lastxs*0.98),(lastydInt*0.98));
   if (ydInt<638){
   line((xs*1.02),(ydInt*1.02),(lastxs*1.02),(lastydInt*1.02));
   }
   //print(xs);
   //print(":");
   //println(ydInt);
